...
export const validateFieldValue = (
  ...
  switch (type) {
    case "email":
      ...
      if (!isMatched)
        throwError(
          `Please enter ${fieldName || "email address"} in valid format.`,
          400
        );
    case "password":
      isMatched = value.match(new RegExp(fieldValitationTypes[type]));
      if (!isMatched)
        throwError(
          `${
            fieldName || "Password"
          } should be of minimum 8 character and should contain atleast one uppercase, lowercase, number and special character.`,
          400
        );
    case "date":
      isMatched = value.match(new RegExp(fieldValitationTypes[type]));
